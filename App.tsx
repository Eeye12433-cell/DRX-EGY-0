...
import { Product, UserProfile, VerificationCode, CartItem, Order, StripeConfig, OrderStatus } from './types';
import { INITIAL_PRODUCTS, GOALS, ACTIVITY_LEVELS } from './constants';
...
const ContactView: React.FC<{lang: 'ar' | 'en'}> = ({ lang }) => (
  <div className="max-w-2xl mx-auto py-20 px-4">
    <h2 className="text-5xl font-oswald uppercase mb-8 text-center">{lang === 'ar' ? 'تواصل' : 'Contact'} <span className="text-drxred">{lang === 'ar' ? 'معنا' : 'Us'}</span></h2>
...
const PolicyView: React.FC<{lang: 'ar' | 'en', type: string}> = ({ lang, type }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto py-20 px-4 prose prose-invert">
      <h2 className="text-5xl font-oswald uppercase mb-12 text-drxred">{lang === 'ar' ? titles[type].ar : titles[type].en}</h2>
...
const App: React.FC = () => {
  ...
  const toggleCompare = (product: Product) => {
    setCompareList(prev => prev.find(p => p.id === product.id) ? prev.filter(p => p.id !== product.id) : prev.length < 4 ? [...prev, product] : prev);
  };
  ...
  return (
    <div className="min-h-screen font-inter overflow-x-hidden selection:bg-drxred selection:text-white bg-bg-primary text-text-main">
      <Navbar onMenuClick={() => setIsSidebarOpen(true)} onCartClick={() => setIsCartOpen(true)} cartCount={cart.reduce((s, i) => s + i.quantity, 0)} lang={lang} setLang={setLang} theme={theme} setTheme={setTheme} onAdminClick={() => navigate('/admin')} />
      ...
      <CartPanel isOpen={isCartOpen} onClose={() => setIsCartOpen(false)} cart={cart} onRemove={removeFromCart} onUpdateQuantity={updateQuantity} onClearCart={clearCart} lang={lang} onCheckout={() => setIsCheckoutOpen(true)} />
      <CheckoutModal isOpen={isCheckoutOpen} onClose={() => setIsCheckoutOpen(false)} cart={cart} stripeConfig={stripeConfig} lang={lang} onOrderComplete={handleOrderComplete} />
      ...
      <main className="pt-24 min-h-[calc(100vh-100px)]">
        <div className="page-content container mx-auto px-4 md:px-8">
          <Routes>
            ...
            <Route 
              path="/shop" 
              element={<Shop lang={lang} products={products} setProducts={setProducts} addToCart={addToCart} onCompare={toggleCompare} compareList={compareList} cart={cart} viewHistoryNames={viewHistoryNames} purchaseHistoryNames={purchaseHistoryNames} />} 
            />
            <Route 
              path="/shop/:mode" 
              element={<Shop lang={lang} products={products} setProducts={setProducts} addToCart={addToCart} onCompare={toggleCompare} compareList={compareList} cart={cart} viewHistoryNames={viewHistoryNames} purchaseHistoryNames={purchaseHistoryNames} />} 
