  ...
  const saveProduct = async (e: React.FormEvent) => {
    ...
    try {
      const payload: Omit<Product, 'id'> = {
        ...
        category: formData.category || Category.Protein,
      ...
      if (editingKey && editingKeyType === 'id') {
        const success = await updateProduct(editingKey, payload);
        if (!success) {
        ...
        if (!newProduct) {
          alert('Failed to deploy new Unit to database.');
          return;
        }
      }
      ...
      setIsFormOpen(false);
      alert('Unit successfully synchronized with database matrix.');
    } catch (err) {
      console.error('Error saving product:', err);
      alert('SYNTAX ERROR: Uplink failed to write data stream.');
    }
  };

  const handleDeleteProduct = async (p: Product) => {
    const name = lang === 'ar' ? p.name_ar : p.name_en;
    if (!window.confirm(`ERASE UNIT "${name}" FROM REGISTRY? This action cannot be undone.`)) return;
    try {
      const success = await deleteProduct(p.id);
      if (success) {
        await refetchProducts();
      } else {
        alert('Failed to delete unit. Check database permissions.');
      }
    } catch (err) {
  ...
  if (!serverIsAdmin) {
    return <div className="max-w-md mx-auto py-32 px-4">
      <div className="card-ui p-10 shadow-2xl text-center">
        ...
        {serverAdminError && <p className="text-[10px] font-mono text-drxred/80 bg-drxred/10 border border-drxred/20 p-3 rounded">
          {serverAdminError}
        </p>}
        <div className="flex gap-3 justify-center mt-8">
          <button onClick={() => verifyAdminServerSide()} className="btn-drx bg-white text-black px-6 py-3 font-bold uppercase tracking-widest text-xs hover:bg-drxred hover:text-white transition-all">
            Retry
          </button>
          <button onClick={handleLogout} className="btn-drx bg-zinc-800 text-white px-6 py-3 font-bold uppercase tracking-widest text-xs hover:bg-drxred transition-all">
            Sign Out
          </button>
        </div>
      </div>
  ...
  return <div className="py-6 space-y-10 pb-24">
    <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 border-b border-ui pb-8">
      ...
      <div className="flex items-center gap-4">
        <div className="flex bg-bg-card p-1 border-ui rounded-sm overflow-x-auto custom-scrollbar border-0">
          ...
          <div className="p-8">
            ...
            <div className="flex justify-between items-end">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={() => handleDeleteProduct(p)} className="text-[10px] font-mono font-bold uppercase text-muted hover:text-red-500 underline decoration-red-500/30 underline-offset-4 tracking-mega transition-colors">
                  Delete
                </button>
              </div>
            </div>
    ...
    {activeTab === 'codes' && <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-bg-card border border-ui p-8 rounded-sm shadow-xl">
        ...
        <button onClick={() => openCodeModal()} className="bg-drxred text-white px-10 py-4 text-[10px] font-black uppercase tracking-mega hover:bg-white hover:text-drxred border border-drxred transition-all shadow-lg">
          + Generate Code
        </button>
        <button onClick={handleBulkGenerate} className="bg-zinc-800 text-white px-10 py-4 text-[10px] font-black uppercase tracking-mega hover:bg-white hover:text-black border border-white/10 transition-all shadow-lg">
          ++ Bulk Generate
        </button>
      </div>

      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
        <input type="text" placeholder="Search codes..." className="bg-transparent border-b border-ui p-3 font-mono text-sm outline-none focus:border-drxred transition-all flex-1" value={codeSearch} onChange={e => setCodeSearch(e.target.value)} />
        <div className="flex gap-2">
          {(['all', 'used', 'available'] as const).map(f => <button key={f} onClick={() => setCodeFilter(f)} className={`px-4 py-2 text-[10px] font-mono font-bold uppercase tracking-mega transition-all ${codeFilter === f ? 'bg-drxred text-white' : 'text-muted hover:text-drxred'}`}>
            {f}
          </button>)}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {filteredCodes.map(c => <div key={c.id} className="card-ui p-6 flex justify-between items-center">
          <div>
          ...
          <div className="flex gap-2">
            <button onClick={() => openCodeModal(c)} className="text-[10px] font-mono font-bold uppercase text-muted hover:text-drxred tracking-mega transition-colors">
              Edit
            </button>
            <button onClick={() => deleteCode(c.id)} className="text-[10px] font-mono font-bold uppercase text-muted hover:text-red-500 tracking-mega transition-colors">
              Delete
            </button>
          </div>
        </div>)}
    ...
    {activeTab === 'orders' && <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {orders.length === 0 ? <div className="text-center py-16 text-muted font-mono">No orders yet.</div> : <div className="space-y-4">
        {orders.map(o => <div key={o.id} className="card-ui p-6">
          <div className="flex flex-col lg:flex-row justify-between gap-4">
            <div className="space-y-2">
              ...
              {o.shippingInfo.address && <div className="text-[10px] font-mono text-muted">{o.shippingInfo.address}</div>}
              <div className="text-xs font-mono text-muted">
                {new Date(o.createdAt).toLocaleString()}
              </div>
              <div className="text-sm font-mono">
                {o.items.map((item, i) => <span key={i}>{item.product.name_en} x{item.quantity}{i < o.items.length - 1 ? ', ' : ''}</span>)}
              </div>
            </div>
            <div className="flex flex-col items-end gap-3">
              ...
              <select value={o.status} onChange={e => updateOrderStatus(o.id, e.target.value as OrderStatus)} className="bg-transparent border border-ui p-2 font-mono text-xs uppercase">
    ...
    {/* Product Form Modal */}
    {isFormOpen && <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 overflow-y-auto">
        ...
        <form onSubmit={saveProduct} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <input type="text" className="w-full bg-transparent border-b border-ui p-3 font-mono text-sm outline-none focus:border-drxred" value={formData.name_en || ''} onChange={e => setFormData(prev => ({
                ...prev,
            ...
            <div className="space-y-2">
              ...
              <input type="text" className="w-full bg-transparent border-b border-ui p-3 font-mono text-sm outline-none focus:border-drxred" value={formData.name_ar || ''} onChange={e => setFormData(prev => ({
                ...prev,
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <textarea className="w-full bg-transparent border border-ui p-3 font-mono text-sm outline-none focus:border-drxred min-h-[80px]" value={formData.description_en || ''} onChange={e => setFormData(prev => ({
                ...prev,
            ...
            <div className="space-y-2">
              ...
              <textarea className="w-full bg-transparent border border-ui p-3 font-mono text-sm outline-none focus:border-drxred min-h-[80px]" value={formData.description_ar || ''} onChange={e => setFormData(prev => ({
                ...prev,
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-2">
              ...
              <input type="number" className="w-full bg-transparent border-b border-ui p-3 font-mono text-sm outline-none focus:border-drxred" value={formData.price || 0} onChange={e => setFormData(prev => ({
                ...prev,
            ...
            <div className="space-y-2">
              ...
              <select className="w-full bg-transparent border-b border-ui p-3 font-mono text-sm outline-none focus:border-drxred" value={formData.category || Category.Protein} onChange={e => setFormData(prev => ({
                ...prev,
            ...
            <div className="space-y-2">
              ...
              <input type="number" className="w-full bg-transparent border-b border-ui p-3 font-mono text-sm outline-none focus:border-drxred" value={formData.featured || 0} onChange={e => setFormData(prev => ({
                ...prev,
                featured: Number(e.target.value)
              }))} />
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <input type="url" className="w-full bg-transparent border-b border-ui p-3 font-mono text-sm outline-none focus:border-drxred" value={formData.image || ''} onChange={e => setFormData(prev => ({
              ...prev,
              image: e.target.value
            }))} placeholder="https://..." />
            {formData.image && <div className="mt-2 w-32 h-32 bg-black overflow-hidden rounded">
          ...
          <div className="flex flex-wrap gap-4">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={formData.inStock ?? true} onChange={e => setFormData(prev => ({
                ...prev,
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={formData.isNew ?? false} onChange={e => setFormData(prev => ({
                ...prev,
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={formData.isBestSeller ?? false} onChange={e => setFormData(prev => ({
                ...prev,
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              {GOALS.map(g => <button key={g.id} type="button" onClick={() => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <input type="text" className="w-full bg-transparent border-b border-ui p-3 font-mono text-sm outline-none focus:border-drxred" value={formData.sizes?.join(', ') || ''} onChange={e => setFormData(prev => ({
                ...prev,
            ...
            <div className="space-y-2">
              ...
              <input type="text" className="w-full bg-transparent border-b border-ui p-3 font-mono text-sm outline-none focus:border-drxred" value={formData.flavors?.join(', ') || ''} onChange={e => setFormData(prev => ({
                ...prev,
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <input type="number" step="0.1" min="0" max="5" className="w-full bg-transparent border-b border-ui p-3 font-mono text-sm outline-none focus:border-drxred" value={formData.rating || 5} onChange={e => setFormData(prev => ({
                ...prev,
            ...
            <div className="space-y-2">
              ...
              <input type="number" min="0" className="w-full bg-transparent border-b border-ui p-3 font-mono text-sm outline-none focus:border-drxred" value={formData.reviews || 0} onChange={e => setFormData(prev => ({
                ...prev,
          ...
          <div className="border border-ui p-6 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <select className="w-full bg-transparent border-b border-ui p-3 font-mono text-sm outline-none focus:border-drxred" value={selectedTheme} onChange={e => setSelectedTheme(e.target.value)}>
                {BG_THEMES.map(t => <option key={t.id} value={t.prompt}>{t.label}</option>)}
              </select>
            </div>
            <button type="button" onClick={handleSynthesizeImage} disabled={isGeneratingImage} className="bg-drxred text-white px-6 py-3 text-[10px] font-bold uppercase tracking-mega disabled:opacity-50">
              {isGeneratingImage ? 'Generating...' : 'Generate Image'}
            </button>

            <div className="flex gap-2 items-end">
              <input type="text" placeholder="Edit prompt (e.g. 'make it darker')" className="flex-1 bg-transparent border-b border-ui p-3 font-mono text-sm outline-none focus:border-drxred" value={aiEditPrompt} onChange={e => setAiEditPrompt(e.target.value)} />
              <button type="button" onClick={handleAiEdit} disabled={isGeneratingImage || !aiEditPrompt} className="bg-zinc-700 text-white px-6 py-3 text-[10px] font-bold uppercase tracking-mega disabled:opacity-50">
                AI Edit
              </button>
            </div>
          ...
          <div className="flex gap-4 pt-4">
            <button type="submit" className="bg-drxred text-white px-10 py-4 text-[10px] font-black uppercase tracking-mega hover:bg-white hover:text-drxred border border-drxred transition-all flex-1">
              {editingKey ? 'Save Modifications' : 'Deploy Unit'}
            </button>
            <button type="button" onClick={() => setIsFormOpen(false)} className="bg-zinc-800 text-white px-10 py-4 text-[10px] font-black uppercase tracking-mega hover:bg-zinc-700 transition-all">
              Cancel
            </button>
          </div>
        </form>
    ...
    {/* Code Modal */}
    {isCodeModalOpen && <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
      <div className="card-ui max-w-md w-full p-8">
        ...
        <form onSubmit={saveCode} className="space-y-6">
          <div className="space-y-2">
            ...
            <input type="text" className="w-full bg-transparent border-b border-ui p-3 font-mono text-sm outline-none focus:border-drxred" value={codeFormId} onChange={e => setCodeFormId(e.target.value)} disabled={!!editingCodeId} required />
          </div>
          <label className="flex items-center gap-2 cursor-pointer">
          ...
          <div className="flex gap-4">
            <button type="submit" className="bg-drxred text-white px-8 py-3 text-[10px] font-black uppercase tracking-mega flex-1">
              {editingCodeId ? 'Update' : 'Create'}
            </button>
            <button type="button" onClick={() => setIsCodeModalOpen(false)} className="bg-zinc-800 text-white px-8 py-3 text-[10px] font-black uppercase tracking-mega">
              Cancel
            </button>
          </div>
        </form>
