...
const Shop: React.FC<ShopProps> = ({
  ...
  // ✅ NEW: Expand/collapse product description on title click
  const [expandedId, setExpandedId] = useState<string | null>(null);

  const getName = (p: Product) => (lang === 'ar' ? p.name_ar : p.name_en);
  const getDesc = (p: Product) => (lang === 'ar' ? (p as any).description_ar : (p as any).description_en);
    ...
    return (
      <div className="py-10 animate-in fade-in duration-500">
        <h2 className="text-7xl font-black font-oswald uppercase mb-16 tracking-tighter drop-shadow-sm">
          {mode === 'goals'
            ? (lang === 'ar' ? 'تسوق حسب الهدف' : 'Shop by Goal')
          ...
          return (
            ...
            >
              <div className="aspect-square mb-8 overflow-hidden relative bg-black">
                ...
                {/* Optional: synthesize button (if you have it somewhere else, ignore) */}
                <button
                  onClick={(e) => handleSynthesizeImage(e, p)}
                  className="absolute top-5 right-5 z-20 px-4 py-2 bg-black/60 border border-white/20 text-white hover:bg-white hover:text-black transition-all text-[10px] font-mono uppercase font-black tracking-widest"
              ...
              <div className="flex-1 flex flex-col justify-between">
                <div>
                  ...
                  {/* ✅ Title click toggles detailed description */}
                  <button
                    type="button"
                    onClick={(e) => {
                      e.stopPropagation(); // مهم: يمنع الـ navigate
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <span
                        className={`mt-4 text-xs transition-transform ${expandedId === p.id ? 'rotate-180' : ''}`}
                        aria-hidden
                      >
                        ▾
                      </span>
                    ...
                    <div className="mt-2">
                      <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-[0.25em] font-bold">
                        {expandedId === p.id
                          ? (lang === 'ar' ? 'إخفاء الوصف' : 'Hide details')
                          : (lang === 'ar' ? 'عرض الوصف' : 'View details')}
                      </span>
                    </div>
                  </button>

                  {/* ✅ Detailed description appears when expanded */}
                  {expandedId === p.id && (
                    <div className="mt-6 border-t border-white/10 pt-4 animate-in fade-in duration-300">
                      <p className="text-sm leading-relaxed text-zinc-300 whitespace-pre-line">
                        {getDesc(p) || (lang === 'ar' ? 'لا يوجد وصف متاح حالياً.' : 'No description available yet.')}
                      </p>
                    </div>
                  )}
                </div>
